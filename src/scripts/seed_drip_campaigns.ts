...
async function main() {
  console.log('Seeding default drip campaigns...');

  for (const campaign of campaigns) {
    // Check if a campaign for this day already exists
    const existing = await prisma.dripCampaign.findFirst({
      where: { daysAfterSignup: campaign.daysAfterSignup }
    });

    if (existing) {
      console.log(`Campaign for day ${campaign.daysAfterSignup} already exists, updating...`);
      ...
      console.log(`Creating campaign for day ${campaign.daysAfterSignup}...`);
  ...
  console.log('Finished seeding drip campaigns!');
}
