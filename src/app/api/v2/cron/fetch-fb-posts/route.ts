...
export async function GET(req: Request) {
	try {
		...
		// 1. Fetch posts from Facebook Graph API (this is usually fast enough to keep awaited, but we'll do the heavy part in the background)
		...
		(async () => {
			console.log(`[FB Fetch Background] Processing ${posts.length} posts...`);
			let addedCount = 0;
			...
			console.log(`[FB Fetch Background] Finished processing. Added ${addedCount} new blogs.`);
		})();

		return NextResponse.json({
			success: true,
			message: `Started processing Facebook posts in the background.`
		});
